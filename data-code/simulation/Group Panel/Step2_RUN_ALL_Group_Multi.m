...
for M    = [2]           
    for N    = [50 ]          % 50, 100
        for T    = [100]          % 50, 100
            ...
            DataFolder = fullfile( sprintf(append(pathhtc, 'DATA/DATA_FE_Tdist_Multi', '_N',num2str( N ),'T', num2str( T )  )) );

            % folder to store results
            ResultFolder = fullfile( sprintf(append(pathhtc, 'RESULT/RESULT_FE_Multi', '_c_', num2str( est.c ), '_c1_', num2str( est.c1 ), '_clambda_', num2str( est.clambda0 ), '_cN_', num2str( est.cN ), '_cT_', num2str( est.cT ), ...
